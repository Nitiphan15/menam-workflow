/*
    Refresh current rows from dbo.delivery_plan_data into dbo.delivery_plan_data.
    ...
    - Identity values are preserved when delivery_plan_data has an identity column.
...
IF OBJECT_ID(N'dbo.delivery_plan_data', N'U') IS NULL
BEGIN
    RAISERROR('Missing target table dbo.delivery_plan_data', 16, 1);
    RETURN;
END;

BEGIN TRANSACTION;

DELETE FROM dbo.delivery_plan_data;

DECLARE @live_object_id int = OBJECT_ID(N'dbo.delivery_plan_data');
DECLARE @dev_object_id int = OBJECT_ID(N'dbo.delivery_plan_data');
...
BEGIN
    ...
    RAISERROR('No matching insertable columns for delivery_plan_data -> delivery_plan_data', 16, 1);
    RETURN;
END;
...
IF @has_identity = 1
    SET @sql = @sql + N'SET IDENTITY_INSERT dbo.delivery_plan_data ON;';

SET @sql = @sql
    + N'INSERT INTO dbo.delivery_plan_data (' + @cols + N') '
    + N'SELECT ' + @cols + N' FROM dbo.delivery_plan_data;';

IF @has_identity = 1
    SET @sql = @sql + N'SET IDENTITY_INSERT dbo.delivery_plan_data OFF;';

EXEC sys.sp_executesql @sql;

COMMIT TRANSACTION;

SELECT COUNT_BIG(*) AS delivery_plan_data_rows
