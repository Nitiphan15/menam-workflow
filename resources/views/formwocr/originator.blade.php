...
@section('content')
    @php
        ...
        $action = $action ?? route('wocr.store');
        $sku = $sku ?? '';
    ...
    <div class="sheet">
        <form method="POST" action="{{ $action }}" enctype="multipart/form-data" id="originatorForm">
            @csrf
            ...
            <div class="border rounded-3 p-3 mb-3">
                <div class="mb-3">
                    ...
                    <div class="form-check form-check">
                        <input class="form-check-input @error('req_type') is-invalid @enderror" type="radio"
                            name="req_type" id="req_type1" value="1">
                        <label class="form-check-label" for="inlineRadio1">ขอให้เปิดใบคำสั่งผลิตใหม่</label>
                    </div>
                    <div class="form-check form-check">
                        <input class="form-check-input @error('req_type') is-invalid @enderror" type="radio"
                            name="req_type" id="req_type2" value="2">
                        <label class="form-check-label" for="inlineRadio2">ขอให้แก้ไขใบคำสั่งผลิต</label>
                    </div>
                    <div class="form-check form-check @error('req_type') is-invalid @enderror">
                        <input class="form-check-input" type="radio" name="req_type" id="req_type3" value="3">
                        <label class="form-check-label" for="inlineRadio2">ยกเลิกใบคำสั่งผลิต</label>
                    </div>
                    @error('req_type')
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" id="form_type" name="form_type"
                            class="form-control @error('form_type') is-invalid @enderror" value="{{ old('form_type') }}"
                            readonly>
                        @error('form_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    ...
                    <div class="mb-3">
                        <textarea name="mfg_request_detail" rows="3" class="form-control" placeholder="อธิบายเหตุผล">{{ old('mfg_request_detail') }}</textarea>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const fill = (m = {}) => {
                ...
                typeEl.value = m.type ?? '';
                sizeEl.value = m.size ?? 0;
                qtyEl.value = m.wo_qty ?? 0;
                lengthEl.value = m.length ?? 0;
            };
            ...
            const form = document.getElementById('originatorForm');
