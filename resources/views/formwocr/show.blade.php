...
@section('content')
    @php
        ...
        $statusOptions =
            $statusOptions ??
            collect([
                1 => 'Originator Request',
                2 => 'Planner Request',
                3 => 'Manager Planner Approval',
                999 => 'Closed',
                998 => 'Voided',
            ]);
            ...
            <div class="col-6 col-sm-3 col-md-2 col-lg-2">
                ...
                <select name="status" class="form-select">
                    ...
                    @foreach ($statusOptions as $code => $label)
                        <option value="{{ $code }}" @selected((string) $status === (string) $code)>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
                        ...
                        @forelse($list as $i => $row)
                            @php
                                ...
                                if ($canApprove) {
                                    ...
                                    $openUrl = route('wocr.view', $row->form_id ?? ($row->wfForm?->id ?? $row->id));
                                }
                            @endphp
                            <tr>
