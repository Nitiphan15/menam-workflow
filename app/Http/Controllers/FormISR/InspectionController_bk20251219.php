...
class InspectionController extends Controller
{
    ...
    protected function buildViewModel(string $mfgNo): ?array
    {
        ...
        $normalizeDefectValue = function ($val, array $cfg) {
            if (!($cfg['defect_master'] ?? false)) {
                return $val; // ไม่ใช่ defect ก็คืนค่าปกติ
            }

            if ($val === null || $val === '') return 'ไม่ได้เลือก';

            // ทำให้เป็น int ชัวร์ ๆ
            $code = is_numeric($val) ? (int)$val : null;
            if ($code === null) return $val;

            return $this->defectMaster[$code] ?? $val; // ถ้าไม่มีใน master ก็โชว์ค่าดิบ
        };
        ...
        foreach ($coilKeys as $coilNorm) {
            ...
            if (count($wtList) === 0) {
                ...
                foreach ($columnConfig as $colKey => $cfg) {
                    ...
                    // ✅ เก็บ hardness points ให้ blade ใช้ทำ UI สวย ๆ
                    if ($colKey === 'hardness') {
                        $row->hardness_points = $this->getHardnessPointsFromQa($qaItem);
                        $row->hardness_avg    = $this->calcHardnessAvgFromQa($qaItem);
                    }
                }
        ...
        $specText   = trim($remark . "\n" . $stepDetail);
        $specCache  = $this->buildSpecCache($remark, $stepDetail); // tensile/elongation
        $colSpec    = [];
        ...
        foreach ($columnConfig as $colKey => $cfg) {
            ...
            if (isset($extractors[$colKey])) {
                $colSpec[$colKey] ??= $extractors[$colKey]([
                    ...
                    'spec_text' => $specText,
                    'spec_cache' => $specCache, // ใช้สำหรับ tensile/elongation
                ]);
        ...
        // --------------------------------------------------
        // 10) Summary เฉพาะ ขนาด / Tensile
        // --------------------------------------------------
        $decideBySpec = function (?float $avg, array $col) {
            if ($avg === null) return null;

            $lsl = $col['lsl'] ?? null;
            $usl = $col['usl'] ?? null;
            $std = $col['standard'] ?? null;
            $dec = $col['decimals'] ?? 3;

            // มี upper/lower → เช็คช่วง
            if (is_numeric($lsl) || is_numeric($usl)) {
                if (is_numeric($lsl) && $avg < (float)$lsl) return 'NG';
                if (is_numeric($usl) && $avg > (float)$usl) return 'NG';
                return 'OK';
            }

            // ไม่มี upper/lower → เทียบกับ standard (ถ้าเป็นตัวเลข)
            if (is_numeric($std)) {
                $a = round($avg, (int)$dec);
                $s = round((float)$std, (int)$dec);
                return ($a === $s) ? 'OK' : 'NG';
            }

            // standard เป็นข้อความ (เช่น "240-280") แต่ parse ไม่ได้ → ไม่ตัดสิน
            return null;
        };
        ...
        // helper
        $calcSummary = function (array $vals) {
            $n = count($vals);
            if ($n === 0) {
                return ['avg' => null, 'max' => null, 'min' => null, 'stdev' => null, 'decide' => null];
            }
            ...
            return ['avg' => $avg, 'max' => $max, 'min' => $min, 'stdev' => $stdev, 'decide' => null];
        };
        ...
        foreach ($displayColumns as $col) {
            ...
            foreach ($qaRows as $r) {
                ...
                // ✅ defect: ถ้าเป็นข้อความ "OK/ปกติ" ให้ถือว่า 0
                if ($zeroIsOk) {
            ...
            $summaryCols[$key] = $calcSummary($vals);
            $summaryCols[$key]['decide'] = $decideBySpec(
                is_numeric($summaryCols[$key]['avg'] ?? null) ? (float)$summaryCols[$key]['avg'] : null,
                array_merge($col, ['decimals' => $cfg['decimals'] ?? ($col['decimals'] ?? 3)])
            );
        }
    ...
    protected function getAllColumnDefs(): array
    {
        return [
            ...
            'coil_dia' => [
                'label' => 'ขนาดวงลวด',
                'keywords' => ['ขนาดวงลวด', 'วงลวด', 'Coil', 'coil dia', 'coil diameter'],
                'value_sources' => ['wot_by_master', 'spec_text'], // ✅ สำคัญ: เอาค่าวัดก่อน
                'fallback_group' => 'vqt',
                'decimals' => 0,
            ],
            ...
            'hardness' => [
                'label' => 'Hardness',
                'keywords' => ['Hardness', 'HV', 'ความแข็ง'],
                'qawip_field' => '__hardness_avg',
                'value_sources' => ['qawip_calc'],
                'decimals' => 2,
            ],
    ...
    {
        return [
            // ขนาด: ใช้ wowc เป็นหลัก + fallback remark
            'size' => function ($ctx) {
                $wowc = $ctx['wowc'];
                $remark = $ctx['remark'];

                $standard = $wowc->msize ?? null;
                $usl = $wowc->msizetolp ?? null;
                $lsl = $wowc->msizetolm ?? null;

                if ($standard === null) {
                    $seg = $this->extractSpecSegment($remark, 'ขนาด');
            ...
            // ความยาว: ใช้ remark ก่อน ถ้าไม่มี → fallback std_mlen
            'length' => function ($ctx) {
                $specText = ($ctx['spec_text'] ?? '') ?: trim(($ctx['remark'] ?? '') . "\n" . ($ctx['step_detail'] ?? ''));

                // ดึง segment หลัง keyword ความยาว / length
                $seg = $this->extractSpecSegmentByKeyword($specText, ['ความยาว', 'Length', 'mlen'], 'มม');

                // parse "2,000 มม +20.00 -0.00"
                $parsed = $this->parseDimWithTolerance($seg);
                ...
                // fallback ถ้ายังไม่เจอจริง ๆ
                if ($standard === null) {
                    $test = $ctx['test'] ?? null;
                    if ($test && ($test->std_mlen ?? null) !== null && $test->std_mlen !== '') {
                        $standard = $test->std_mlen;
                    }
                }
            ...
            // Tensile: ดึงเฉพาะหลังคำ Tensile/TS เท่านั้น (กัน Resin 200-300)
            'tensile' => function ($ctx) {
                ...
                // fallback เผื่อ cache ไม่ได้ (กันพัง)
                $seg = $this->extractSpecSegmentByKeyword($ctx['remark'] ?? '', ['Tensile', 'TS', 'เทนไซล์'], 'N/mm');
                if ($seg === null) {
                    $seg = $this->extractSpecSegmentByKeyword($ctx['step_detail'] ?? '', ['Tensile', 'TS', 'เทนไซล์'], 'N/mm');
                }
            ...
            'coil_dia' => function ($ctx) {
                $seg = $this->extractSpecSegmentByKeyword(
                    ...
                    ['ขนาดวงลวด', 'วงลวด', 'Coil', 'coil dia', 'coil diameter'],
                    'มม'
                );

                // seg จะได้ประมาณ "600-1000 มม" หรือ "600-1000"
                $parsed = $this->parseSpecValueRobust($seg);
            ...
            'elongation' => function ($ctx) {
                ...
                $seg = $this->extractSpecSegmentByKeyword($ctx['remark'] ?? '', ['Elongation', 'EL', 'ยืดตัว'], '%');
                if ($seg === null) {
                    $seg = $this->extractSpecSegmentByKeyword($ctx['step_detail'] ?? '', ['Elongation', 'EL', 'ยืดตัว'], '%');
                }
                $parsed = $this->parseSpecValueRobust($seg);
            ...
            'hardness' => function ($ctx) {
                // ดึงจาก remark/step_detail เช่น "Hardness : 240-280 HV"
                $seg = $this->extractSpecSegmentByKeyword(
                    ($ctx['remark'] ?? '') . ' ' . ($ctx['step_detail'] ?? ''),
                    ['Hardness', 'HV', 'HRC', 'แข็ง'],
                    'HV'
                );
                $parsed = $this->parseSpecValueRobust($seg);
            ...
            'roundness' => function ($ctx) {
                $txt = $this->normalizeRemark(
                    (($ctx['remark'] ?? '') . ' ' . ($ctx['step_detail'] ?? ''))
                );

                // ✅ เอา "เลขตัวแรกหลังคำว่า ความกลม/roundness" เท่านั้น
                if (preg_match('/ความกลม[^0-9]*([0-9]+(?:\.[0-9]+)?)/u', $txt, $m)) {
                    return ['standard' => $m[1], 'usl' => null, 'lsl' => null];
                }
                if (preg_match('/\broundness\b[^0-9]*([0-9]+(?:\.[0-9]+)?)/iu', $txt, $m)) {
                    return ['standard' => $m[1], 'usl' => null, 'lsl' => null];
                }

                // fallback: ถ้าไม่มีในข้อความ ให้ใช้ std_roundness จาก wot
                $test = $ctx['test'] ?? null;
                if ($test && isset($test->std_roundness) && $test->std_roundness !== '') {
                    return ['standard' => (string)$test->std_roundness, 'usl' => null, 'lsl' => null];
                }

                return ['standard' => null, 'usl' => null, 'lsl' => null];
            },

            // ถ้าจะเพิ่ม speed/time/ra → มาเติมที่นี่ได้เลย (ไม่ต้องแก้ loop)
        ];
    }
    ...
    protected function parseDimWithTolerance(?string $text): array
    {
        ...
        // fallback: เจอแค่ตัวเลขเดียว
        if (preg_match('/([\d,\.]+)/', $text, $m)) {
            return [
                'standard' => (float) str_replace(',', '', $m[1]),
                'usl' => null,
                'lsl' => null,
            ];
        }
    ...
    protected function extractSpecSegmentByKeyword(string $text, array $aliases, ?string $preferUnit = null): ?string
    {
        ...
        foreach ($aliases as $kw) {
            // ดึงข้อความหลัง keyword 1..80 ตัว (กันลากไปกินตัวเลขชุดอื่น)
            $pattern = '/(?:^|[\(\[\s])' . preg_quote($kw, '/') . '\s*[:：\-–—]?\s*([^;\)\]\n\r]{1,120})/iu';
            ...
            if (preg_match_all($pattern, $hay, $mm, PREG_OFFSET_CAPTURE)) {
                foreach ($mm[1] as $i => $cap) {
                    ...
                    // ใกล้ keyword ในต้น ๆ ให้คะแนนเพิ่มนิดหน่อย
                    $score += max(0, 10 - (int)($pos / 50));

                    $candidates[] = ['seg' => $seg, 'score' => $score];
                }
            }
        }
    ...
    protected function parseSpecValueRobust(?string $text): array
    {
        ...
        // Min / Max
        if (preg_match('/\bmin(?:imum)?\b\s*([0-9]+(?:\.[0-9]+)?)/iu', $clean, $m)) {
            return ['standard' => "Min {$m[1]}", 'lsl' => (float)$m[1], 'usl' => null];
        }
        if (preg_match('/\bmax(?:imum)?\b\s*([0-9]+(?:\.[0-9]+)?)/iu', $clean, $m)) {
            return ['standard' => "Max {$m[1]}", 'lsl' => null, 'usl' => (float)$m[1]];
        }

        // > / <
        if (preg_match('/[>≥]\s*(-?\d+(?:\.\d+)?)/u', $clean, $m)) {
            return ['standard' => '>' . $m[1], 'lsl' => (float)$m[1], 'usl' => null];
        }
        if (preg_match('/[<≤]\s*(-?\d+(?:\.\d+)?)/u', $clean, $m)) {
            return ['standard' => '<' . $m[1], 'lsl' => null, 'usl' => (float)$m[1]];
        }
    ...
    protected function resolveValueBySources(
        ...
        foreach (($cfg['value_sources'] ?? []) as $src) {
            ...
            if ($src === 'qawip_calc') {
                if (($cfg['qawip_field'] ?? '') === '__hardness_avg') {
                ...
                $val = ($qField && $qaSample) ? ($qaSample->{$qField} ?? null) : null;
            } elseif ($src === 'spec_text') {
                $seg = $this->extractSpecSegmentByKeyword(
                    (string)$specText,
                    $cfg['keywords'] ?? [],
                    'มม'
